...
class GameScreen:
    ...
    def _load_icons(self):
        ...
        icon_map = {
            ...
            'hint': ['hint.png', 'bulb.png', 'lightbulb.png'],
            'check': ['check.png', 'verify.png'],
            'solve': ['solve.png', 'play_solve.png', 'puzzle.png'],
            'clear': ['clear.png', 'eraser.png'],
            'new': ['new.png', 'plus.png', 'add.png'],
            'pause': ['pause.png'],
            'resume': ['play.png', 'resume.png'],
            'back': ['back.png', 'arrow_left.png']
        }
    ...
    def _create_header(self):
        ...
        self.back_button = ttkb.Button(header_frame,
                                       text=("Quay lại" if back_icon is None else ""),
                                       image=back_icon, command=self._on_back, bootstyle="danger")
        self.back_button.pack(side="left")

        ttkb.Label(header_frame, text="SUDOKU", font=("Arial", 20, "bold"),
                   bootstyle="primary").pack(side="top", pady=4)
    ...
    def _create_info_panel(self):
        ...
        lives_label = ttkb.Label(info_frame, textvariable=self.lives_var,
                                 font=("Arial", 12, "bold"), bootstyle="danger")
    ...
    def _create_board(self):
        ...
        cell_font_size = 16 if self.grid_size == 9 else 12
        cell_width = 2 if self.grid_size == 9 else 1
        cell_pixel_width = 45 if self.grid_size == 9 else 30
        cell_pixel_height = 45 if self.grid_size == 9 else 30
    ...
    def _create_controls(self):
        ...
        help_btn = ttkb.Button(controls_frame, text=("Hướng dẫn" if help_icon is None else ""),
        ...
        hint_btn = ttkb.Button(controls_frame, text=("Gợi ý" if hint_icon is None else ""),
                               image=hint_icon, command=self._on_hint,
                               bootstyle="primary-outline", width=6)
        hint_btn.pack(side="left", padx=10)

        check_icon = self._get_icon('check')
        check_btn = ttkb.Button(controls_frame, text=("Kiểm tra" if check_icon is None else ""),
        ...
        solve_btn = ttkb.Button(controls_frame, text=("Giải" if solve_icon is None else ""),
                                image=solve_icon, command=self._on_solve,
                                bootstyle="purple", width=6)
        solve_btn.pack(side="left", padx=10)

        clear_icon = self._get_icon('clear')
        clear_btn = ttkb.Button(controls_frame, text=("Xóa" if clear_icon is None else ""),
                                image=clear_icon, command=self._on_clear,
                                bootstyle="danger-outline", width=6)
        clear_btn.pack(side="left", padx=10)

        new_icon = self._get_icon('new')
        new_game_btn = ttkb.Button(controls_frame, text=("Mới" if new_icon is None else ""),
        ...
        self.pause_btn = ttkb.Button(controls_frame,
                                     text=("Tạm dừng" if pause_icon is None else ""),
                                     image=pause_icon,
                                     command=self._toggle_pause, bootstyle="secondary", width=10)
    ...
    def _toggle_pause(self):
        ...
        if not getattr(model, 'is_paused', False):
            ...
            if resume_icon is not None:
                self.pause_btn.config(text='', image=resume_icon)
            ...
            if pause_icon is not None:
                self.pause_btn.config(text='', image=pause_icon)
                self.pause_btn.image = pause_icon
            else:
                self.pause_btn.config(text='Tạm dừng')
            self.pause_btn.config(bootstyle='secondary')
